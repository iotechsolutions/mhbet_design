...
class Project
{
	...
	public function addProject($name, $description, $paths, $images)
	{
		$uploadDir = "../photo/project/";
		...
		// Build the SQL query
		$sql = "INSERT INTO projects (`name`, `description`, Path1, Path2, Path3) VALUES (?, ?, ?, ?, ?)";
	...
	public function updateProject($ID, $name, $description, $paths, $images, $pathOlds)
	{
		$uploadDir = "../photo/project/";
		// Handle path parameters
		$fileNames = [];
	...
	{
		try {
			$stmt = $this->con->query("SELECT * FROM projects");
			if (!$stmt) {
	...
	//Remove project
	public function removeproject($ID, $paths)
	{
