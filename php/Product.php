...
class Product
{
	...
	public function addProduct($name, $description, $price, $paths, $images)
	{
		$uploadDir = "../photo/product/";
		...
		// Build the SQL query
		$sql = "INSERT INTO products (`name`, `description`, price, Path1, Path2, Path3) VALUES (?, ?, ?, ?, ?, ?)";
		$params = array_merge([$name, $description, $price], $fileNames);
		...
		if ($result) {
			...
			// document.location = '../admin/products.php' </script>";
		} else {
			echo "<script>alert('Adding Failed! Please check your connection');
			document.location = '../admin/products.php' </script>";
		}
	}
	public function updateProduct($ID, $name, $description, $price, $paths, $images, $pathOlds)
	{
		$uploadDir = "../photo/product/";
		// Handle path parameters
		$fileNames = [];
		...
		$params = array_merge([$name, $description, $price], $fileNames, [$ID]);
		$stmt = $this->con->prepare($sql);
		$result = $stmt->execute($params);
	...
	//Remove project
	public function removeproduct($ID, $paths)
	{
